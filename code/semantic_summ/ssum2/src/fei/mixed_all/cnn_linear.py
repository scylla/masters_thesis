...
if __name__ == '__main__':
	...
	input_dir = '/Users/gohan/Desktop/Thesis/semantic_summ/semantic_summ/src/fei/mixed_all/cnn/'

	cats = ['Test', 'Train', 'Validation']
	for _cat in cats:
		for ty in ['art', 'summ']:
			# global edge node info
			global_info = GlobalNodeEdgeInfo()
			text_graph_dict = {} # all linearized graph representations
			body_file = ty + '_' + _cat + '_ft.jamr'
			buildLinearCorpusAndWriteToFile(os.path.join(input_dir, body_file), output_file='output_file')
			print_graph_stats()
			linearized_graph_dict = {}
			with codecs.open(input_dir + _cat + '_' + ty + '.txt', 'wb', 'utf-8') as output_file:
				for filename in text_graph_dict:
					sent_list, summ_list = text_graph_dict[filename].create_text_sum_data(_depth)
					linearized_graph_dict[filename] = sent_list
					for sent in sent_list:
						output_file.write("%s\n" % sent)
			with codecs.open(input_dir + _cat + '_' + ty + '_meta.pkl', 'wb') as meta_f:
				pickle.dump(linearized_graph_dict, meta_f)