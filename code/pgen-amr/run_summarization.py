...
from data import Vocab, POSVocab
...
tf.app.flags.DEFINE_integer('pos_emb_dim', 128, 'pos dimension of word embeddings')
...
tf.app.flags.DEFINE_float('pos_loss_weight', 0.5, 'weight loss for pos')
...
def main(unused_argv):
  ...
  hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm', 'pos_loss_weight', 'hidden_dim', 'pos_emb_dim', 'emb_dim', 'batch_size', 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt', 'pointer_gen']
  hps_dict = {}
  ...
  batcher = Batcher(FLAGS.data_path, vocab, pos_vocab, hps, single_pass=FLAGS.single_pass)

  tf.set_random_seed(111) # a seed value for randomness

  if hps.mode == 'train':
    print "creating model..."
    model = SummarizationModel(hps, vocab, pos_vocab)
    ...
    model = SummarizationModel(decode_model_hps, vocab, pos_vocab)
